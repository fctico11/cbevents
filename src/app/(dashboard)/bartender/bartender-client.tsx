...
import {
    ...
    ChevronRight, ChevronLeft, Check, Clock, Sparkles,
} from "lucide-react";
import { createRequest } from "@/lib/supabase/actions";
...
export function BartenderClient({ event, bar, userName, requestTree, requests }: Props) {
    ...
    const handleNodeTap = useCallback((node: any) => {
        if (node.is_terminal) {
        ...
        >
            ...
            <div className="mb-6">
                ...
                <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
                    {requestTree.filter((n) => n.is_terminal).slice(0, 4).map((node) => (
                        <button
                            key={node.id}
                    ...
                    {requestTree.length === 0 && (
                        <p className="text-sm text-muted-foreground">No request buttons configured yet.</p>
                    )}
                </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {currentNodes.map((node: any) => (
                        ...
                        >
                            ...
                            {!node.is_terminal && (
                                <span className="text-xs text-muted-foreground">{node.children?.length || 0} items</span>
                            )}
                        </button>
                    ))}
                </div>
...
function RecentRequestCard({ request }: { request: any }) {
    return (
